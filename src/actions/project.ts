...
export const createProject = async (projectData: ProjectData): Promise<CreateProjectResponse> => {
    ...
    if (projectData.project_end_date) {
        if (!validateDate(projectData.project_end_date)) {
            errors.push({ field: 'project_end_date', message: 'Invalid date format' })
        }
    }
    ...
    if (projectData.skills && projectData.skills.length > 0) {
        projectData.skills.forEach((skill) => {
            ...
            if (trimmedSkill.length > 50) {
                errors.push({ field: 'skills', message: `Skill "${trimmedSkill}" must be 50 characters or less` })
    ...
    if (projectData.imageData && projectData.imageData.length > 0) {
        projectData.imageData.forEach((imageData, index) => {
            if (!imageData.id || typeof imageData.id !== 'string') {
                errors.push({ field: 'images', message: `Invalid UUID at position ${index + 1}` })
            } else if (!imageData.blobUrl || !validateBlobUrl(imageData.blobUrl)) {
                errors.push({ field: 'images', message: `Invalid blob URL at position ${index + 1}` })
            } else if (imageData.caption && imageData.caption.length > 255) {
                errors.push({ field: 'images', message: `Caption at position ${index + 1} must be 255 characters or less` })
            } else {
    ...
    try {
        ...
        if (validImageData.length > 0) {
            ...
            const imageLinkRecords: TablesInsert<'project_image_link'>[] = imageIds.map(imageId => ({
            ...
            const imageRecords: TablesInsert<'project_image'>[] = validImageData.map(image => ({
                id: image.id,
            ...
            if (imageError) {
                ...
                await supabase.from('project_image_link').delete().eq('project_id', projectId)
                await supabase.from('project').delete().eq('id', projectId)
                return {
                    success: false,
        ...
        if (validSkills.length > 0) {
            ...
            if (skillIds.length > 0) {
                const skillLinks: TablesInsert<'project_skill_link'>[] = skillIds.map(skillId => ({
                    project_id: projectId,
                    skill_id: skillId
                }))
