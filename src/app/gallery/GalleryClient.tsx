...
import { GalleryPhoto, CollectionWithPhotoCount, getPhotosByCollection } from '../lib/gallery-collections';
...
export default function GalleryClient({ initialPhotos, collections }: GalleryClientProps) {
  const [currentPhotos, setCurrentPhotos] = useState<GalleryPhoto[]>(initialPhotos);
  ...
  const handleCollectionSelect = async (collectionId: string, collectionName: string) => {
  ...
  return (
    <>
      ...
      <main className="flex max-h-dvh flex-col overflow-hidden font-work-sans fixed inset-0">
        ...
        <div className="flex-shrink-0 py-4 px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-xl sm:text-2xl lg:text-3xl text-foreground">
              {activeCollectionId ?
                collections.find(c => c.id === activeCollectionId)?.name || 'Gallery'
                : 'Featured'
              }
            </h1>
          </div>
