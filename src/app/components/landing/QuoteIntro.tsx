...
export default function QuoteIntro({ onComplete }: QuoteIntroProps) {
  ...
  const clause1 = "There is never a time in the future in which we will work out our salvation.";
  ...
  useGSAP(() => {
    ...
    if (!clause1Element || !clause2Element || !clause3Element || !attributionElement || !containerElement) return;

    // Create SplitText instances for each clause
    const split1 = SplitText.create(clause1Element, { type: "words, lines", wordsClass: "quote-word" });
    const split2 = SplitText.create(clause2Element, { type: "words, lines", wordsClass: "quote-word" });
    const split3 = SplitText.create(clause3Element, { type: "words, lines", wordsClass: "quote-word" });
    ...
    tl.from(split1.lines, {
      ...
      .to([clause1Element, clause2Element, attributionElement], {
        opacity: 0,
        duration: 1.5,
      }, "+=1.2")
